...
using System.Configuration;
    ...
    public class ShareholderStatusFilterer
    {
        ...
        private static DataTable FindStatus(String status)
        {
            ...
            SqlCommand Com = new SqlCommand(@"SELECT sh.first_name, sh.last_name, se.name AS 'stock_exchange', shse.status FROM share_holders sh inner join share_holder_stock_ex shse on sh.share_holder_id = shse.share_holder_id inner join stock_exchanges se on se.stock_ex_id = shse.stock_ex_id where shse.status ='" + status + "' order by se.name", Cn);
            Cn.Open();
